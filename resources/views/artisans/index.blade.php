    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('artisans.create') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold shadow">
        ...
        <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white rounded-xl shadow-sm border border-neutral-100 p-5 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-primary-800">{{ $artisans->count() }}</p>
                </div>
        ...
        @if ($artisans->isEmpty())
            <div class="bg-white rounded-2xl shadow-sm border border-dashed border-neutral-300 p-10 text-center">
                ...
                <a href="{{ route('artisans.create') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold">
                    <i class="fas fa-user-plus mr-2"></i>
                    Add Artisan
                </a>
            </div>
                            ...
                            @foreach ($artisans as $artisan)
                                <tr class="hover:bg-neutral-50/70">
                                    <td class="px-4 py-3 align-top">
                                        <div class="font-semibold text-neutral-900">{{ $artisan->name }}</div>
                                        @if ($artisan->address)
                                            <div class="text-xs text-neutral-500 mt-0.5 truncate max-w-xs">{{ $artisan->address }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $artisan->email }}</td>
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $artisan->phone }}</td>
                                    <td class="px-4 py-3 align-top">
                                        @php
                                        ...
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                            ...
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $status === 'active' ? 'bg-emerald-500' : 'bg-amber-400' }}"></span>
                                            {{ $statusLabel }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 align-top text-right whitespace-nowrap">
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('artisans.show', $artisan) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-neutral-200 text-xs font-semibold text-neutral-700 hover:bg-neutral-50">
                                                <i class="fas fa-eye mr-1.5"></i> View
                                            </a>
                                            <a href="{{ route('artisans.edit', $artisan) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-primary-200 text-xs font-semibold text-primary-700 bg-primary-50 hover:bg-primary-100">
                                                <i class="fas fa-edit mr-1.5"></i> Edit
                                            </a>
                                            <form action="{{ route('artisans.destroy', $artisan) }}" method="POST" class="inline-flex" onsubmit="return confirm('Are you sure you want to delete this artisan?');">
                                                @csrf
                                                @method('DELETE')
