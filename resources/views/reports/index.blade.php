                            ...
                            @foreach($reports as $report)
                                <tr class="hover:bg-neutral-50/50 transition">
                                    <td class="px-6 py-4 align-top text-neutral-900 font-semibold flex items-center gap-2">
                                        @if(str_contains($report->report_type, 'sales'))
                                            <i class="fas fa-receipt text-primary-600"></i>
                                            Sales Report
                                        @elseif(str_contains($report->report_type, 'stock'))
                                    ...
                                    <td class="px-6 py-4 align-top">
                                        ...
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold
                                            {{ $isGenerated ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200' }}">
                                            <span class="w-2 h-2 rounded-full mr-2 {{ $isGenerated ? 'bg-emerald-500' : 'bg-amber-500' }}"></span>
                                            {{ $isGenerated ? 'Ready' : 'Processing' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 align-top text-right">
                                        <a href="{{ route('reports.show', $report) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-primary-300 text-primary-700 bg-primary-50 hover:bg-primary-100 text-sm font-semibold transition">
                                            <i class="fas fa-download"></i>
                                            Download
                                        </a>
                                    </td>
