...
<div class="bg-gradient-to-br from-neutral-50 via-neutral-100 to-neutral-50 min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10">
            ...
            <a href="{{ Auth::guard('artisan')->check() ? route('artisans.dashboard') : route('admin.dashboard') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-lg border border-neutral-300 text-neutral-700 hover:bg-neutral-100 text-sm font-medium transition">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 p-6 hover:shadow-lg transition">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-primary-800">{{ $totalSales }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 p-6 hover:shadow-lg transition">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-secondary-800">RM {{ number_format($totalRevenue, 2) }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 p-6 hover:shadow-lg transition">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-accent-800">{{ $totalProducts }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 p-6 hover:shadow-lg transition">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-primary-700">{{ \App\Models\Artisan::count() }}</p>
                    </div>
                    <span class="inline-flex items-center justify-center w-12 h-12 rounded-lg bg-primary-100 text-primary-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <form method="POST" action="{{ route('reports.sales') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="date" name="start_date" required value="{{ date('Y-m-01') }}" class="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700 uppercase mb-2">End Date</label>
                                <input type="date" name="end_date" required value="{{ date('Y-m-d') }}" class="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition">
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 overflow-hidden hover:shadow-lg transition">
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('reports.stock') }}" class="space-y-4">
                        @csrf
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 overflow-hidden hover:shadow-lg transition">
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('reports.performance') }}" class="space-y-4">
                        @csrf
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700 uppercase mb-2">Start Date</label>
                                <input type="date" name="start_date" required value="{{ date('Y-m-01') }}" class="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:border-secondary-500 focus:ring-2 focus:ring-secondary-200 transition">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-neutral-700 uppercase mb-2">End Date</label>
                                <input type="date" name="end_date" required value="{{ date('Y-m-d') }}" class="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:outline-none focus:border-secondary-500 focus:ring-2 focus:ring-secondary-200 transition">
                            </div>
                        </div>
                        ...
                        <tbody class="divide-y divide-neutral-100">
                            ...
                            <tr class="hover:bg-neutral-50/50 transition">
                                <td class="px-6 py-4 align-top text-neutral-900 font-semibold flex items-center gap-2">
                                    ...
                                    <i class="fas fa-receipt text-primary-600"></i> Sales Report
                                    @elseif($report->report_type == 'stock')
                                    <i class="fas fa-boxes text-accent-600"></i> Stock Report
                                    @else
                                    <i class="fas fa-chart-line text-secondary-600"></i> Performance Report
                                    @endif
                                </td>
                                <td class="px-6 py-4 align-top text-neutral-700">{{ $report->created_at->format('d M Y, H:i') }}</td>
                                <td class="px-6 py-4 align-top">
                                    @if(($report->status ?? null) == 'generated')
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                                        <span class="w-2 h-2 rounded-full mr-2 bg-emerald-500"></span> Ready
                                    </span>
                                    @else
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                        <span class="w-2 h-2 rounded-full mr-2 bg-amber-500"></span> Processing
                                    </span>
                                    @endif
                                </td>
