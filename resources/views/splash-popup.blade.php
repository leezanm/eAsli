    ...
    <div class="relative w-full max-w-2xl mx-4 bg-white rounded-3xl shadow-2xl animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-8 md:p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="relative group rounded-2xl overflow-hidden bg-white border-2 border-neutral-200 hover:border-primary-400 transition-all duration-300 hover:shadow-xl {{ $index == 1 ? 'md:col-span-2 md:row-span-2' : '' }}">
                    <!-- Product Image -->
                    <div class="relative w-full {{ $index == 1 ? 'h-96' : 'h-48' }} bg-gradient-to-br from-neutral-200 to-neutral-300 overflow-hidden">
                    ...
                    <div class="p-4 flex flex-col">
                        <p class="text-xs text-neutral-600 font-semibold uppercase tracking-wider mb-1">{{ ucfirst($product->category) }}</p>
                        <h3 class="text-lg font-bold text-neutral-900 line-clamp-2 mb-2 group-hover:text-primary-700 transition">{{ $product->name }}</h3>
                        ...
                        <p class="text-sm text-neutral-600 line-clamp-2 mb-3 flex-grow">{{ Str::limit($product->description, 80) }}</p>

                        <div class="flex items-center justify-between gap-2">
                            <p class="text-2xl font-bold text-primary-700">RM {{ number_format($product->price, 2) }}</p>
                            <a href="{{ route('products.show', $product) }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-primary-600 to-primary-700 text-white text-xs font-bold hover:shadow-lg transition">
