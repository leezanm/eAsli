    ...
    <div class="max-w-7xl mx-auto px-4">
            ...
            <form method="GET" action="{{ route('sales.index') }}" class="space-y-4">
                <div class="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    @if(!empty(array_filter($filters ?? [])))
                        <a href="{{ route('sales.index') }}" class="text-xs font-medium text-primary-700 hover:text-primary-800">
                            Clear filters
                        </a>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    @if(isset($artisans) && $artisans->isNotEmpty())
                        <div>
                            ...
                            <select name="artisan_id" class="w-full px-3 py-2 rounded-lg border border-neutral-300 text-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-200 outline-none">
                                ...
                                @foreach($artisans as $artisan)
                                    <option value="{{ $artisan->id }}" {{ ($filters['artisan_id'] ?? '') == $artisan->id ? 'selected' : '' }}>
                                        {{ $artisan->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-100 p-5 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-primary-800">RM {{ number_format($averageTransaction, 2) }}</p>
                </div>
                <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary-50 text-primary-600">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-100 p-5 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-primary-800">{{ $salesToday }}</p>
                </div>
                <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary-50 text-primary-600">
        ...
        @if($sales->isEmpty())
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-dashed border-neutral-300 p-10 text-center">
                ...
                @auth('artisan')
                    <a href="{{ route('sales.create') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold">
                        <i class="fas fa-plus mr-2"></i>
                        Record Sale
                    </a>
                @endauth
                        ...
                        <tbody class="divide-y divide-neutral-100">
                            @foreach($sales as $sale)
                                <tr class="hover:bg-neutral-50/70">
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $sale->created_at->format('d/m/Y H:i') }}</td>
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $sale->product->name }}</td>
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $sale->customer->name }}</td>
                                    <td class="px-4 py-3 align-top text-neutral-800">{{ $sale->quantity }}</td>
                                    <td class="px-4 py-3 align-top font-semibold text-primary-800">RM {{ number_format($sale->total_price, 2) }}</td>
                                    <td class="px-4 py-3 align-top">
                                        ...
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                            ...
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $isCompleted ? 'bg-emerald-500' : 'bg-amber-400' }}"></span>
                                            {{ $statusLabel }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 align-top text-right whitespace-nowrap">
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('sales.show', $sale) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-neutral-200 text-xs font-semibold text-neutral-700 hover:bg-neutral-50">
                                                <i class="fas fa-eye mr-1.5"></i> View
                                            </a>
                                            @auth('artisan')
                                                <a href="{{ route('sales.edit', $sale) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-primary-200 text-xs font-semibold text-primary-700 bg-primary-50 hover:bg-primary-100">
