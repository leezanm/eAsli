...
<div class="max-w-6xl mx-auto px-4 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-md border border-neutral-200 p-6">
            ...
            <p class="text-3xl font-bold text-accent-600">RM {{ number_format($totalSpent, 2) }}</p>
        </div>
        <div class="bg-white rounded-xl shadow-md border border-neutral-200 p-6">
            <p class="text-sm font-semibold text-neutral-600 mb-1">Average Order</p>
            <p class="text-3xl font-bold text-secondary-600">RM {{ $totalOrders > 0 ? number_format($totalSpent / $totalOrders, 2) : '0.00' }}</p>
        </div>
    </div>
    ...
    @if($sales->isNotEmpty())
        <div class="bg-white rounded-2xl shadow-md border border-neutral-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-neutral-200 bg-neutral-50 flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-neutral-600">{{ $sales->total() }} order{{ $sales->total() !== 1 ? 's' : '' }}</span>
            </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        @if($sale->product->image_path)
                                            <img src="{{ Storage::url($sale->product->image_path) }}" alt="{{ $sale->product->name }}" class="w-10 h-10 object-cover rounded">
                                        @else
                                        ...
                                        @endif
                                        <div>
                                            <p class="font-semibold text-neutral-900">{{ $sale->product->name }}</p>
                                            <p class="text-xs text-neutral-500">{{ $sale->product->category }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-neutral-700">
                                    {{ $sale->artisan->name ?? 'Unknown' }}
                                </td>
                                <td class="px-6 py-4 text-neutral-700">
                                    {{ $sale->quantity }}
                                </td>
                                <td class="px-6 py-4 text-neutral-700">
                                    RM {{ number_format($sale->unit_price, 2) }}
                                </td>
                                <td class="px-6 py-4 text-right font-semibold text-neutral-900">
                                    RM {{ number_format($sale->total_price, 2) }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold {{ $sale->payment_status === 'paid' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">
                                        <span class="w-2 h-2 rounded-full {{ $sale->payment_status === 'paid' ? 'bg-emerald-600' : 'bg-amber-600' }}"></span>
    ...
    @else
        <div class="bg-white rounded-2xl border border-neutral-200 shadow-md p-12 text-center">
            ...
            <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-gradient-to-r from-accent-500 to-accent-600 text-white text-sm font-semibold shadow-md hover:from-accent-600 hover:to-accent-700 transition">
