        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    @foreach($items as $line)
                        ...
                        <div class="py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div class="flex items-start gap-4 flex-1">
                                ...
                                <div>
                                    <h3 class="text-sm font-semibold text-neutral-900">{{ $product->name }}</h3>
                                    <p class="text-xs text-neutral-500 mt-1">Category: {{ ucfirst($product->category) }}</p>
                                    <p class="text-xs text-neutral-500 mt-1">Unit price: RM {{ number_format($line['unit_price'], 2) }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
                                <form method="POST" action="{{ route('cart.update', $product) }}" class="flex items-center gap-2">
                                    @csrf
                                    <input type="number" name="quantity" min="1" value="{{ $line['quantity'] }}" class="w-16 px-2 py-1 border border-neutral-300 rounded-lg text-sm text-center">
                                ...
                                <div class="text-right">
                                    <p class="text-sm font-semibold text-neutral-900">RM {{ number_format($line['total'], 2) }}</p>
                                    <form method="POST" action="{{ route('cart.remove', $product) }}" class="mt-1">
                                        @csrf
            ...
            <div class="bg-white rounded-2xl shadow-md border border-secondary-200 p-6 flex flex-col justify-between">
                ...
                @if(auth('customer')->check())
                    ...
                    <a href="{{ route('cart.checkout') }}" class="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-lg bg-gradient-to-r from-primary-600 to-primary-500 text-white text-sm font-semibold shadow-md hover:from-primary-700 hover:to-primary-600 transition mt-4">
